...
export async function handleMissingInvoiceData(input: ExtractInvoiceDataInput): Promise<ExtractInvoiceDataOutput> {
  return handleMissingInvoiceDataFlow(input);
}

const handleMissingInvoiceDataPrompt = ai.definePrompt({
  name: 'handleMissingInvoiceDataPrompt',
...
export const handleMissingInvoiceDataFlow = ai.defineFlow(
  {
    name: 'handleMissingInvoiceDataFlow',
    inputSchema: ExtractInvoiceDataInputSchema,
    outputSchema: ExtractInvoiceDataOutputSchema,
  },
  async input => {
    const {output} = await handleMissingInvoiceDataPrompt(input);
